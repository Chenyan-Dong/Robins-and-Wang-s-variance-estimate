...
#Initialize variables to store simulation results
	RubinSE1<-RobinsWangSE1<-RubinSE2<-RobinsWangSE2<-Estimate1<-Estimate2<-AnalysisN<-NULL
	output<-matrix(NA,sims,6)
	colnames(output)<-c("Estimate1", "Estimate2","RW1", "RW2","RR1", "RR2")
...
#Fixed values
  ...
  #Original dataset
	dat<-dat.original<-data.frame(ID,X1,X2,Y)
	dat$Intercept<-1

	# randomly sample missing on x
	missrate <- 0.3
	miss1 <- with(dat, sample(ID, 0.15 * obs))
	miss2 <- with(dat, sample(ID, 0.15 * obs))
  ...
  # Fit linear regression model using complete observations
	...
	mod1<-glm(X1~Y,data=dat,family="gaussian" ,y=FALSE,model=FALSE)
	mod2<-glm(X2~X1+Y,data=dat,family="gaussian" ,y=FALSE,model=FALSE)
	...
	imputed.X1<-ImputationFn(mod1,datty=dat.imputed)
	dat.imputed$X1[miss1]<-imputed.X1[miss1]
	imputed.X2<-ImputationFn(mod2,datty=dat.imputed)
	dat.imputed$X2[miss]<-imputed.X2[miss]
...
RW_Components_Imputation_Fn<-function(){
	...
	S_u_mod1<-S_u_function(dat.imputed,mod1,dat.imputed[,rownames(attributes(mod1$terms)$factors)[1]])
	S_u_mod2<-S_u_function(dat.imputed,mod2,dat.imputed[,rownames(attributes(mod2$terms)$factors)[1]])
	...
	S2_mod1<-S2_function(dat.imputed,mod1,dat.imputed[,rownames(attributes(mod1$terms)$factors)[1]])
	S2_mod2<-S2_function(dat.imputed,mod2,dat.imputed[,rownames(attributes(mod2$terms)$factors)[1]])
...
for (p in 1:Nimpute){
	...
	mod.analysis<-glm(Y~X1+X2,data=dat.analysis,family="binomial")	
	...
	analysis.est1[p]<-mod.analysis$coef["X1"]
	analysis.se1[p]<-sqrt(diag(vcov(mod.analysis))["X1"])
	analysis.est2[p]<-mod.analysis$coef["X2"]
	analysis.se2[p]<-sqrt(diag(vcov(mod.analysis))["X2"])
...
###############################################################
	...
	RobinsWangSE1[k]<-sqrt(diag(GAMMA)["X1"])
	RobinsWangSE2[k]<-sqrt(diag(GAMMA)["X2"])
	RubinSE1[k]<-sqrt(mean(analysis.se1^2)+(Nimpute+1)/Nimpute*var(analysis.est1))			
	RubinSE2[k]<-sqrt(mean(analysis.se2^2)+(Nimpute+1)/Nimpute*var(analysis.est2))			
	
	#Other values to output
	#Parameter Estimate
	Estimate1[k]<-mean(analysis.est1)
	Estimate2[k]<-mean(analysis.est2)

  output[k,]<-c(Estimate1[k], Estimate2[k],RobinsWangSE1[k], RobinsWangSE2[k],RubinSE1[k],RubinSE2[k])
}

out <- colMeans(output)
results <- data.frame(Estimate1 = rep(out[1], 2), SE1 = out[3:4], lowerCI1 = c(out[1] - out[3:4]), 
                   upperCI1 = c(out[1] + out[3:4]), 
                   Estimate2 = rep(out[2], 2), SE2 = out[5:6],  lowerCI2 = c(out[2] - out[5:6]), 
                   upperCI2 = c(out[2] + out[5:6])) %>% 
  mutate(widthCI1 = upperCI1 - lowerCI1, widthCI2 = upperCI2 - lowerCI2) 
Results <- results[, c("Estimate1", "SE1", "lowerCI1", "upperCI1", "widthCI1",
                 "Estimate2", "SE2", "lowerCI2", "upperCI2", "widthCI2")]
rownames(Results) <- c("RW", "Rubin")
