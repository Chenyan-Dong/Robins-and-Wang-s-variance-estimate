---
title: "Function for logistic regression"
...
impute <- function(data, imputemod){
  ...
  if(imputemod$family == "binomial"){
    ...
    predicted.values <- predict(imputemod, newdata = impute.data, returnvar = "linpred")
    S_u <- impute.data[, imputemod.vars]*(imputedvars - exp(predicted.values)/(1 + exp(predicted.values)))
    modvarcount <- length(imputemod.vars)
    
    imputeX <- impute.data[, imputemod.vars]
    # predicted.values is actually predicted LP
    predicted.values <- predict(imputemod, newdata = impute.data, type = "link")
	  ...
	  for (matrow in 1:ncol(imputeX)){
	    for (matcol in 1:ncol(imputeX)){
	      ...
	      temp[impute.data$Imputed == 1] <- 0
	      S2_mod[matrow, matcol] <- sum(temp)/length(impute.data$id)
	    }
	  }
	  }else{
	  desmatrix <- rmvnorm(1, mean = imputemod$coefficients, sigma = vcov(imputemod))
    ...
    modvarcount <- length(imputemod.vars) + 1
