...
#Fit linear regression model using complete observations
	...
	imputemod.vars<-c("Intercept",all.vars(formula(mod)[-2]))
	imputedvar <- dat.imputed[, rownames(attributes(mod$terms)$factors)[1]]
	predicted.values<-predict(mod,newdata=dat.imputed,returnvar="linpred")
	S_u<-dat.imputed[,imputemod.vars]*(imputedvar-exp(predicted.values)/(1+exp(predicted.values)))
...
for (p in 1:Nimpute){
	...
	mod.analysis<-glm(Y~X,data=dat.analysis,family="binomial")	
