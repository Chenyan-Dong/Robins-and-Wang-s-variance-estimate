---
title: "X follows normal dist Y follows logistic model"
...
We fit a logistic regression model of `Y` on `X` is given by $\textrm{logit} \, \mathrm{E}(\textrm{Y | X} = x)  = \beta_0 + \beta_1 x + \varepsilon$ as the analysis model.

```{r, analysis, eval = FALSE}
# fit logistic regression model (analysis model)
fit.analysis <- glm(y ~ x, data.impute, family = "binomial")
...
imputation <- function(data, nimpute, obs) {
  ...
  seRobinsWang <- mean(results[2,])
  seRubin <- mean(results[3,])
  
  output <- c(estimate, seRobinsWang, seRubin)
	names(output) <- c("estimate", "seRobinsWang", "seRubin")
	output
}
```
