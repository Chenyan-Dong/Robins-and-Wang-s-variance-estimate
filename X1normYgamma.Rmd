---
title: "X1 follows normal dist Y follows gamma regression"
...
`Y` follows the gamma distribution with shape parameter $a = 1$ and scale parameter $b$ is the same as an exponential distribution of scale parameter (or mean) $b$. When $a$ is greater than 1, the Gamma distribution assumes a mounded (unimodal), but skewed shape. The skewness reduces as the value of $a$ increases. Where $b = \frac{\mu}{a}$, b is 
The $\mu$ could be calculated through the `exp` function, which follows
$$\textrm{log} \, \mu = \beta_0 + \beta_1 x$$
$$\mu = e^{\beta_0 + \beta_1 x}$$
We set the association between `x` and `y` should be\
$$\textrm{log} \, \mathrm{E}(\textrm{Y | X} = x)  = \beta_0 + \beta_1 x + \varepsilon$$
...
# y follow the gamma distribution 
mu <- exp(Xmat %*% beta)
gscale <- mu/gshape
...
miss <- with(data, sample(id, 0.3 * obs))
data[miss, "x"] <- NA
...
We fit a log gamma regression model of `Y` on `X` is given by $\textrm{log} \, \mathrm{E}(\textrm{Y | X} = x)  = \beta_0 + \beta_1 x + \varepsilon$ as the analysis model.

```{r, analysis, eval = FALSE}
# fit log gamma regression model (analysis model)
fit.analysis <- glm(y ~ x, data.impute, family = Gamma(log))
...
$$u_i(\hat{\theta}, \hat{\beta}_k) = (y - \beta_0 - \beta_1 x)x$$
...
analysis.predLP <- predict(fit.analysis, newdata = tempdat, type = "response")
